...
module.exports.getConfig = () => {
  ...
  const globalOverrides = requireOptional(`/${home}/.neu-component-config.json`)

  const localOverrides = requireOptional(
    `/${currentPath}/.neu-component-config.json`
  )
...
module.exports.buildPrettifier = (prettierConfig) => {
  ...
  if (!config) {
    ...
    try {
      config = fs.readFileSync(path.join(currentPath, '/.prettierrc.json'), {
        encoding: 'utf8',
        flag: 'r',
      })
    } catch (err) {
...
module.exports.logConclusion = (componentName) => {
  console.info('\n')
  console.info(
    chalk.bold.rgb(...colors.green)(`✨ Component ${componentName} created ✨`)
  )
  console.info('\n')
}
